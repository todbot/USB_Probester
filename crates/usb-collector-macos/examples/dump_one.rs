/// Dump USB devices using the real MacCollector — validates nusb + ioreg HID pass.
...
fn print_device(d: &UsbDevice, depth: usize) {
    ...
    println!("{indent}{:?} device @ {} ({})  \"{}\"",
        d.speed, d.location_id,
        format!("{:04X}:{:04X}", dd.id_vendor, dd.id_product),
        product);

    println!("{indent}  Device Descriptor ({} raw bytes):",
        dd.raw_bytes.len());
    ...
    if !d.hid_interfaces.is_empty() {
        for hid in &d.hid_interfaces {
            println!("{indent}  HID Report Descriptor ({} bytes):",
