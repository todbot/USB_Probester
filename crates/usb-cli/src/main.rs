...
fn main() {
    ...
    match cli.format {
        ...
        Format::Json => {
            println!("{}", serde_json::to_string_pretty(&devices).expect("serialize failed"));
        }
    }
}
...
fn print_interface(d: &UsbDevice, iface: &InterfaceDescriptor) {
    ...
    if let Some(name) = string_for(d, iface.i_interface) {
        ...
        println!("        Interface #{} - {}   ", iface.b_interface_number, class_sub);
    }
...
fn print_endpoint(ep: &EndpointDescriptor) {
    ...
    println!("                Polling Interval:   {} ms", ep.b_interval);
}
...
fn format_pkt_size(size: u16, xfer: u8) -> String {
    if xfer == 1 || xfer == 3 {
        ...
        format!(
            "0x{:04X}  ({} x {} transactions per microframe)",
            size, mult, pkt
        )
    } else {
        format!("{}", size)
